...
inoz(このリポジトリのコードと、ビルド済みの `index.html`)は
**[PolyForm Noncommercial License 1.0.0](LICENSE)** で提供します。

- **非商用なら自由です。**個人・学校・研究機関・自治体などの公的機関・非営利団体は、
  複製・改変・再配布・自分のサーバでの運用を含めて、許諾を求めずに使えます
  (条文は政府機関・教育機関・公共の安全/保健機関を非商用に明示的に含めています)
- **商用利用は別途の許諾が必要です。**製品やサービスへの組み込み、有償の受託での利用、
  自治体向けに業者が納めるといった場合は[GitHub の Issue](https://github.com/tomoakiitoh/inoz/issues) でご相談ください(専用の窓口は追って用意します)
- 複製・改変物には、この条件(または上のURL)と、次の1行を残してください
  `Required Notice: Copyright (c) 2026 Tomoaki Itoh (https://inoz.jp)`
- 「inoz」という名前とロゴの使用は、このライセンスに含まれません。改変版を配るときは別の名前にしてください
- 無保証です。特に、浸水や避難の判断にこの道具の表示を使わないでください(上の「謳わないこと」の表)

これは OSI の定義するオープンソースではなく、いわゆる **source-available** です。
外部からのプルリクエストは当面受け付けていません(受けるときは CLA を用意します)。

### あなたが作ったもの

景色を指す URL、貼り付けたデータ、画面の記録や動画は**あなたのもの**です。inoz は権利を主張しません。
ただし写っている地形・写真・建物は国土地理院と PLATEAU のデータなので、
公開するときは**それぞれの出典表示**(上の「出典」)に従ってください。
